<?php
    ...
    $response = json_decode($referralController->getAllPaidReferrals(), true);
...
<div class="subMenu">
    <span>
        ...
        <a class="active" href="./referral.php?page=0">Paid</a>
                    ...
                    if(is_array($response["message"]) && count($response["message"]) > 0){
                        foreach($response["message"] as $key => $referral){
                            ...
                            <tr>
                                ...
                                <td> <span data-referred="<?php echo $referral["referral_referredby"]; ?>" data-id="<?php echo $referral["referral_id"]; ?>" class="paid btnReferralPaid">Paid</span> </td>
                            </tr>
                            <?php
                        }
                    }else{
                        ?>
                        <tr> <td colspan="6">Ooooops! We don't currently have a referral to be paid...</td> </tr>
                        <?php
                    }
                ?>
            </tbody>
