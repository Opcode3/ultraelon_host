<?php
    ...
    $response = json_decode($withdrawController->getAllPendingWithdraws(), true);
    // var_dump($response);
?>
                    ...
                    if(is_array($response["message"]) && count($response["message"]) > 0){
                        foreach($response["message"] as $key => $withdraw){
                            ...
                            <tr>
                                ...
                                <td> <span data-id="<?php echo (int) $withdraw["withdraw_id"]; ?>" class="pending btnWithdrawPending">Pending</span> </td>
                            </tr>
                            <?php
                        }
                    }else{
